...
def startReceiveingClockTime(connector, address):
    while True:
...
def startConnecting(master_server):
    ...
    while True:
        ...
        current_thread = threading.Thread(target = startReceiveingClockTime, args = (master_slave_connector, slave_address, ))
        ...
        if len(client_data) > 0:
            ...
            for client_addr, client in client_data.items():
                try:
                    ...
                    client['connector'].send(synchronized_time.strftime('%Y-%m-%d %H:%M:%S').encode())
